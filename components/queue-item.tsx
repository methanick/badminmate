...
interface QueueItemProps {
  ...
  onStart: (queueId: string, courtId: string) => void;
...
export function QueueItem({
  ...
  const isPlaying = queue.courtId !== undefined && queue.courtId !== null;

  const availableCourts = courts.filter(
    (c) => !c.isPlaying || c.id === queue.courtId,
  );
              ...
              {[0, 1].map((slotIndex) => (
                <PlayerSlot
                  ...
                  onAddPlayer={(playerId) =>
                    onAddPlayer(queue.id, "team1", slotIndex, playerId)
                  }
                  disabled={readOnly}
                />
              ))}
            ...
            <div className="flex gap-1 min-w-0">
              {[0, 1].map((slotIndex) => (
                <PlayerSlot
                  ...
                  onAddPlayer={(playerId) =>
                    onAddPlayer(queue.id, "team2", slotIndex, playerId)
                  }
                  disabled={readOnly}
                />
              ))}
                  ...
                  {isPlaying ? (
                    ...
                    <>
                      ...
                      <Button
                        onClick={() => {
                          if (selectedCourtId) {
                            onStart(queue.id, selectedCourtId);
                          } else {
